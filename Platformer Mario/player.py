...
class Player(pygame.sprite.Sprite):
    ...
    def import_character_assets(self):
        characer_path = 'graphics/character/'
        self.animations = {'idle': [], 'run': [], 'jump': [], 'fall': []}

        for animation in self.animations.keys():
            full_path = characer_path + animation
            self.animations[animation] = import_folder(full_path)

    def import_dust_particles(self):
        self.run_dust_particles = import_folder(
            'graphic/character/dust_particles/run')
    ...
    def run_dust_animation(self):
        if self.status == 'run' and self.on_ground:
            ...
            if self.dust_frame_index > len(self.run_dust_animation):
                self.dust_frame_index = 0
